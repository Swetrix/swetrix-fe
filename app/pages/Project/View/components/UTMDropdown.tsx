...
const UTMDropdown = ({ onSelect, title }: IUTMDropdown): JSX.Element => {
  ...
  return (
    ...
    >
      ...
      {show && (
        <div className='absolute z-10 mt-2 left-5 top-15 text-gray-900 bg-white border border-gray-200 rounded-md shadow-lg dark:bg-[#191e2d] dark:border-slate-700/50 min-w-[250px] max-h-[200px] overflow-auto'>
          <div className='flex flex-col w-full p-2'>
            ...
            {_map(OPTIONS, ({ label, value }) => (
              <div
                ...
                className='flex flex-row items-center justify-between cursor-pointer w-full px-1 py-2 text-sm text-gray-700 dark:text-gray-200 dark:bg-[#191e2d] hover:bg-gray-200 dark:hover:bg-slate-800'
              >
                <p>{t(label)}</p>
              </div>
            ))}
