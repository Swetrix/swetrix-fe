...
const Overview = ({ overall, chartData, activePeriod, t, live }) => {
  ...
  return (
    <PanelContainer name={t('project.overview')} noSwitch>
      <div className="flex text-lg justify-between">
        ...
        <p className="h-5 mr-2 text-gray-900 dark:text-gray-50 text-xl">{live}</p>
      </div>
      {!_isEmpty(chartData) && (
        <>
...
const CustomEvents = ({ customs, chartData, t }) => {
  ...
  return (
    <PanelContainer
      ...
      <table className="table-fixed">
        <thead>
          <tr>
            <th className="w-4/6 text-left text-gray-900 dark:text-gray-50">
              {t('project.event')}
            </th>
        ...
        <tbody>
          {_map(keys, (ev) => (
            <tr key={ev}>
              <td className="text-left text-gray-900 dark:text-gray-50">{ev}</td>
...
const Panel = ({
  ...
  const keysToDisplay = useMemo(
    () => _slice(keys, currentIndex, currentIndex + 5),
    [keys, currentIndex],
  );
            ...
            <Fragment key={key}>
              <div
                ...
                {linkContent ? (
                  <a
                    className={cx(
                      'flex items-center label hover:underline text-blue-600 dark:text-blue-500',
                      { capitalize },
                    )}
                    href={rowData}
                    target="_blank"
