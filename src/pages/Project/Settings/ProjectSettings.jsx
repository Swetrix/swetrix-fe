...
import { createProject, updateProject, deleteProject, resetProject } from "api";
...
const ProjectSettings = ({
  ...
  const project = useMemo(
    () =>
      _find(
        [...projects, ..._map(sharedProjects, (item) => item.project)],
        (p) => p.id === id
      ) || {},
    [projects, id, sharedProjects]
  );
  const isSettings =
    !_isEmpty(id) && _replace(routes.project_settings, ":id", id) === pathname;
    ...
    if (!projectSaving) {
      ...
      try {
        const formalisedData = {
          ...
          ipBlacklist: _isEmpty(data.ipBlacklist)
            ? null
            : _split(data.ipBlacklist, ","),
        };
        if (isSettings) {
  ...
  const onCancel = () => {
    history.push(
      isSettings ? _replace(routes.project, ":id", id) : routes.dashboard
    );
  };

  const title = isSettings
    ? `${t("project.settings.settings")} ${form.name}`
  ...
  return (
    <Title title={title}>
      <div
        className={cx(
          "min-h-min-footer bg-gray-50 dark:bg-gray-800 flex flex-col py-6 px-4 sm:px-6 lg:px-8",
          {
            "pb-40": isSettings,
          }
        )}
        ...
        <form className="max-w-7xl w-full mx-auto" onSubmit={handleSubmit}>
          ...
          {isSettings ? (
            <>
              ...
              />
              <Checkbox
                checked={Boolean(form.spam_protection)}
                onChange={handleInput}
                name="spam_protection"
                id="spam_protection"
                className="mt-4"
                label="spam_protection"
                hint="spam_protectionHint"
              />
              ...
              {!project.shared && <People project={project} />}
            </>
          ) : (
